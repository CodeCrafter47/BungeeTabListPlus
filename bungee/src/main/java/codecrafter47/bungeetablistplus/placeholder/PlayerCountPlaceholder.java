...
public class PlayerCountPlaceholder extends PlaceholderProvider {
    @Override
    public void setup() {
        bind("server_player_count").to(context -> String.format("%d", context.getPlayerManager().getPlayerCount(context.getServer(), context.getViewer(), BungeeTabListPlus.getInstance().getConfigManager().getMainConfig().showPlayersInGamemode3)));
