...
public abstract class AbstractTabOverlayHandler implements PacketHandler, TabOverlayHandler {
            ...
            if (teamEntry != null) {
                if (packet.getMode() == 0 || packet.getMode() == 2) {
                    ...
                    if (TEAM_COLOR_IS_BYTE) {
                        teamEntry.setColor(PacketUtil.getTeamColorByte(packet));
                    } else {
    ...
    private abstract class CustomContentTabOverlayHandler<T extends CustomContentTabOverlay> extends AbstractContentOperationModeHandler<T> {
        ...
        void update() {
            ...
            if (tabOverlay.dirtyFlagSize) {
                ...
                if (using80Slots) {
                    if (areCustomSlotUsersPartOfTeams) {
                        for (int index = 0; index < 80; index++) {
                            sendPacket(PACKETS_REMOVE_CUSTOM_SLOT_USERS_FROM_TEAMS[index]);
                        }
                    ...
                    if (!areCustomSlotUsersPartOfTeams) {
                        for (int index = 0; index < 80; index++) {
                            sendPacket(PACKETS_ADD_CUSTOM_SLOT_USERS_TO_TEAMS[index]);
                        }
                        areCustomSlotUsersPartOfTeams = true;
                    }
    ...
    private static Team createPacketTeamCreate(String name, String displayName, String prefix, String suffix, String nameTagVisibility, String collisionRule, int color, byte friendlyFire, String[] players) {
        ...
        if (TEAM_COLOR_IS_BYTE) {
            PacketUtil.setTeamColorByte(team, (byte) color);
        } else {
            team.setColor(color);
        }
