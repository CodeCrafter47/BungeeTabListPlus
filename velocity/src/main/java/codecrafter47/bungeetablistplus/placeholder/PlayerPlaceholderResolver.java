...
public class PlayerPlaceholderResolver extends AbstractPlayerPlaceholderResolver {
    ...
    public PlayerPlaceholderResolver(ServerPlaceholderResolver serverPlaceholderResolver, Cache cache) {
        ...
        addPlaceholder("velocity_display_name", create(VelocityData.Velocity_DisplayName, (player, name) -> name == null ? player.getName() : name, TypeToken.STRING));
        addPlaceholder("session_duration_seconds", create(VelocityData.Velocity_SessionDuration, duration -> duration == null ? null : (int) (duration.getSeconds() % 60), TypeToken.INTEGER));
        ...
        addPlaceholder("redisbungee_server_id", create(BTLPVelocityDataKeys.DATA_KEY_RedisBungee_ServerId));
    ...
    private PlaceholderBuilder<?, ?> resolvePermissionPlaceholder(PlaceholderBuilder<Player, ?> builder, List<PlaceholderArg> args, TemplateCreationContext tcc) throws PlaceholderException {
        ...
        if (args.get(0) instanceof PlaceholderArg.Text) {
            ...
            return builder.acquireData(new PlayerPlaceholderDataProviderSupplier<>(TypeToken.BOOLEAN, BTLPVelocityDataKeys.permission(permission), (p, d) -> d), TypeToken.BOOLEAN);
        } else {
    ...
    private static class PermissionDataProvider extends AbstractActiveElement<Runnable> implements PlaceholderDataProvider<Context, Boolean>, ExpressionUpdateListener {
        ...
        protected void onActivation() {
            ...
            permissionDataKey = BTLPVelocityDataKeys.permission(permission.evaluate());
            if (hasListener()) {
