...
public class PlayersByServerComponentView extends PartitionedPlayersView {
    ...
    protected void onActivation() {
        ...
        if (showServers == PlayersByServerComponentConfiguration.ServerOptions.ALL) {
            servers = ProxyServer.getInstance().getAllServers().stream().map((registeredServer -> registeredServer.getServerInfo().getName())).collect(Collectors.toList());
            ...
            for (String serverName : servers) {
                ...
                serverDataHolder.addDataChangeListener(BTLPVelocityDataKeys.DATA_KEY_SERVER_ONLINE, onlineStateUpdateListener);
                Boolean online = serverDataHolder.get(BTLPVelocityDataKeys.DATA_KEY_SERVER_ONLINE);
                if (online == null) {
                    online = true;
                }
                if (online) {
    ...
    protected void onDeactivation() {
        ...
        if (showServers == PlayersByServerComponentConfiguration.ServerOptions.ONLINE) {
            for (String serverName : servers) {
                ...
                DataHolder serverDataHolder = BungeeTabListPlus.getInstance().getDataManager().getServerDataHolder(serverName);
                serverDataHolder.removeDataChangeListener(BTLPVelocityDataKeys.DATA_KEY_SERVER_ONLINE, onlineStateUpdateListener);
            }
        }
    }
    ...
    protected int getInitialSizeEstimate(ComponentView componentView) {
        ...
        if (prioritizeViewerServer) {
            String server = getContext().getViewer().get(VelocityData.Velocity_Server);
